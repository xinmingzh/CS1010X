...
class MyCanvas(Canvas):
    def _canvas_coord(self, p):
        x,y = p[0], p[1]
        return (CANVAS_SIZE['x'] + x*CANVAS_SIZE['width'],
                CANVAS_SIZE['y'] + (1-y)*CANVAS_SIZE['height'])

    def create_line(self, src, dest):
        src, dest = self._canvas_coord(src), self._canvas_coord(dest)
        super().create_line(*(src+dest))

    def create_point(self, p):
        cp = self._canvas_coord(p)        
        super().create_oval(cp[0]-1, cp[1]-1, cp[0]+1, cp[1]+1, fill="black")
...
def get_canvas(title):
    ...
    c = MyCanvas(w, width=WINDOW_SIZE, height=WINDOW_SIZE, relief='groove')
...
## Drawing Functions
def draw_connected(n, curve, c=None):
    points = points_for_curve(n, curve)

    if not c:
        c = get_canvas("draw_connected({},{})".format(n, curve.__name__))
    for i in range(0, n):        
        c.create_line(points[i], points[i+1])

def draw_connected_scaled(n, curve, c=None):
...
def draw_points(n, curve, c=None):
    points = points_for_curve(n, curve)
...
def draw_points_scaled(n, curve, c=None):
    points = scaled_points_for_curve(n, curve)
